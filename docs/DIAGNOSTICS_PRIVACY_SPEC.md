...
> 最后更新：2026-07-07
>
> 最后复核：2026-07-22，P7.11 上下文摘要已纳入同一字段允许/禁止清单。
...
- 最近 AI 调用增加版本化 `context` 摘要，设置页可说明本次选入材料、排除原因、阅读规则、字符/token 预算和缓存命中状态。
- 原始来源 id、阅读项 id 和章节 id 会先转换为单向指纹引用；排除说明只保留种类、原因枚举和数量，不保存自由文本 `detail`。
- 摘要经过前端字段白名单后才写入诊断存储，导出诊断包时仍由 Rust 递归脱敏器执行第二次过滤。
- `test:companion-diagnostics` 使用秘密标记验证正文、笔记、prompt、messages、自定义策略字段和排除说明不能进入摘要或复制文本。
...
- AI 上下文来源引用：单向指纹引用、来源种类、页码、字符数、是否压缩/截短；排除材料只保留种类、原因枚举和计数。不得保存原始 id、自由文本命中说明或对应正文。
...
P6.6 基础诊断与 P7.11 上下文解释均已完成。后续如果诊断包需要包含更多文件或新的正文级来源，应提高 packageVersion，并继续遵守本文档的字段允许清单、禁止清单和二次脱敏规则。
