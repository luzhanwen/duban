...
- 已接入 sidebar 伴读问答第一版：
  ...
  - PDF text layer 选中文字后显示跟随式小按钮，支持“解释这句”和“问导师”；“解释这句”会直接发送解释请求，“问导师”会把页码和引用文本放到输入框上方作为引子，等待用户补充问题。
...
- 新增选句跟随式伴读第一版：捕捉 PDF text layer 选区，在选区上方显示 `解释这句 / 问导师` 浮层；`解释这句` 直接发送解释请求，`问导师` 将选中文本作为引用条放在 sidebar 输入框上方。
...
- 阅读会话已实现三段式骨架、阅读位置保存、PDF 原版页渲染、PDF text layer、选中文本提问浮层和当前章节 sidebar 问答，但尚未支持字体设置、选中高亮持久化和 AI 读后反馈。
