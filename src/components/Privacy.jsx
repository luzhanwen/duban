  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <p className="mt-4 max-w-3xl text-sm leading-7 text-ink-soft">
        读伴是纯前端应用，没有自己的后端账号、云书库或分析服务。你的书籍、笔记、
        阅读进度和 API Key 默认保存在当前浏览器里；当你主动使用 AI 能力时，相关文本会发送给你选择的模型服务商。
      </p>

      <section className="mt-8 grid gap-4 md:grid-cols-2">
        <PrivacyCard
          title="书籍文件"
          stored="原始 PDF/MOBI、提取后的分页文本和封面缓存保存在本机浏览器 IndexedDB。"
          sent="生成整本书导读、章节导读、伴读问答或读后交流时，会把必要的章节文本、当前页文本或抽样文本发送给当前模型服务商。"
          notSent="不会上传到读伴自己的服务器，也不会默认同步到云端。"
        />
        <PrivacyCard
          title="API Key"
          stored="API Key 保存在本机浏览器 IndexedDB 的设置数据里；下载当前配置时，TXT 文件也会包含 API Key。"
          sent="测试连接或调用 AI 时，Key 会作为请求头发送给你选择的模型服务商。自定义 Base URL 时，Key 会发送到你填写的地址。"
          notSent="不会写进代码仓库，不会发送给读伴自己的服务器。"
        />
        <PrivacyCard
          title="笔记与高亮"
          stored="原文摘录、页码、高亮位置、笔记正文和从 AI 回答保存的笔记保存在本机浏览器 IndexedDB。"
          sent="读后交流里如果你选择带入伴读问答和笔记，这些内容会作为上下文发送给当前模型服务商。"
          notSent="不会公开发布，也不会离开当前浏览器，除非你主动触发相关 AI 功能或之后自行导出。"
        />
        <PrivacyCard
          title="聊天记录"
          stored="阅读中的问答和读后交流记录按书籍与阅读项保存在本机浏览器 IndexedDB。"
      ...
      <section className="mt-8 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-7 text-amber-900">
        ...
        <p className="mt-2">
          IndexedDB 不是硬件级安全存储。如果这台设备、浏览器配置文件、浏览器扩展或同源页面脚本不可信，
          本地数据和 API Key 仍可能被读取。建议只在可信设备上使用读伴，并为模型服务设置额度、限额或单独的 API Key。
        </p>
      </section>
      ...
      <section className="mt-6 rounded-xl border border-line bg-paper-card p-5 text-sm leading-7 text-ink-soft">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5">
          ...
          <li>清理浏览器站点数据也会移除读伴保存在当前浏览器里的数据。</li>
